<template>
	<view class="bjx-inputs" :style="getStyle">
		...
		<view class="bjx-con">
			<slot name='left'></slot>
			<input maxlength="20" :disabled="theDisabled" :placeholder="placeholder" v-model="theValue" @input="theInput" @tap='isShow=data.length' @focus="theFocus" @blur="theBlur" autocomplete="off" />
		...
		<view class="bjx-select" v-show="show">
			<view class="data">
				<view class="select-item" :class="'item-'+overflow" v-for="(item, index) in data" :key="index"  @tap="selectItem(item,index)">
					<view class="item-text" :class="{active: getItem(item) == theValue}">{{item.classificationName? item.classificationName: item}}</view>
